...
/*   Updated: 2018/01/24 16:56:15 by rlossy      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void	ft_get_flag_first(t_param *p, const char *format, va_list *ap)
{
	...
	if ((p->flag[1] && (FMT2(format[i]) || FMT3(format[i]))) || p->flag[4])
		p->flag[3] = 0;
}
...
void	ft_get_flag_second(t_param *p, const char *format)
{
	...
	i = -1;
	while (format[++i] && !(FMT(format[i])) && !(FMT2(format[i]))
				&& !(FMT3(format[i])) && format[i] != '%')
	{
